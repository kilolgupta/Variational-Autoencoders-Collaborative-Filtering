...
for u_id in test_user_ids:
	print(test_user2id[u_id])
	m_ids = raw_data[(raw_data.userId == u_id) & (raw_data.rating > 3.5)]['movieId'].tolist()
	movie_indexes = [movie2id[m] for m in m_ids]
	test_rows.extend([test_user2id[u_id] for i in range(len(m_ids))])
	test_cols.extend(movie_indexes)

pickle.dump(test_rows, open("test_rows.file", "wb"))
pickle.dump(test_cols, open("test_cols.file", "wb"))
...
# for each user, m be the number of movies he has rated. randomly sample 20% of this and put it as 0 so as to measure the performance against these
